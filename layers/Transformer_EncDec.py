...
class IntAttention2(nn.Module):
    ...
    def temporal_attn(self, x):
        b, c, n, d = x.shape
        new_x = x.reshape(-1, n, d)
        new_x = self.attention(new_x, new_x, new_x)[0]
        new_x = new_x.reshape(b, c, n, d)
        return new_x
...
class CointAttention(nn.Module):
    ...
    def axial_attn(self, x):
        ...
        new_x = rearrange(x, 'b c n d -> (b n) c d')    # [bs*num_patch, n_vars, d_model]
        new_x = (self.pad_ch(new_x.transpose(-1, -2)).transpose(-1, -2).reshape(-1, self.num_rc, d))
