...
class DataEmbedding_inverted(nn.Module):
    ...
    def forward(self, x, x_mark):
        x = x.permute(0, 2, 1).contiguous()      # [Batch, Variate, Seq_len]
...
class PatchEmbed(nn.Module):
    def __init__(self, args, num_p=1, d_model=None):
        ...
        self.num_p = num_p      # patch数量
        self.patch_len = args.seq_len // self.num_p     # patch长度
        self.d_model = args.d_model if d_model is None else d_model

        self.proj = nn.Sequential(
            nn.Linear(self.patch_len, self.d_model, False),
