...
class PSMSegLoader(Dataset):
    def __init__(self, args, root_path, win_size, step=1, flag="train"):
        ...
        self.scaler = StandardScaler()
        data = pd.read_csv(os.path.join(root_path, 'PSM_train.csv'))
        data = data.values[:, 1:]
        data = np.nan_to_num(data)
        self.scaler.fit(data)
        data = self.scaler.transform(data)
        test_data = pd.read_csv(os.path.join(root_path, 'PSM_test.csv'))
        ...
        self.test_labels = pd.read_csv(os.path.join(root_path, 'PSM_test_label.csv')).values[:, 1:]
        if args.c_in == 1:
            self.train = self.train[:, 0:1]
            self.val = self.val[:, 0:1]
            self.test = self.test[:, 0:1]
        print("train:", self.train.shape)  # [58317, 55]
        print("val:", self.val.shape)
        print("test:", self.test.shape)  # [73729, 55]
        print("test_labels:", self.test_labels.shape)  # [73729, 1]
    ...
    def __len__(self):
        if self.flag == "train":
            ...
            # return self.test.shape[0] // (2 * self.win_size)         # 测试时保证窗口不重叠

            # len = (self.test.shape[0] - 2 * self.win_size) // self.win_size + 1         # 窗口没有重叠
            len = (self.test.shape[0] - 2 * self.win_size) // self.step + 1  # 窗口有重叠
            return len

    def __getitem__(self, index):
        # index = index * self.win_size    # 窗口没有重叠
        index = index * self.step  # 窗口有重叠
        if self.flag == "train":
            ...
            # begin = index * 2 * self.win_size
            # end = index * 2 * self.win_size + self.win_size
            # label_begin = index * 2 * self.win_size + self.win_size
            # label_end = index * 2 * self.win_size + 2 * self.win_size
            # return (np.float32(self.test[begin:end]),
            #         np.float32(self.test_labels[label_begin:label_end]))
            return (np.float32(self.test[index:index + self.win_size]),
                    np.float32(self.test[index + self.win_size: index + 2 * self.win_size]),
                    np.float32(self.test_labels[index + self.win_size:index + 2 * self.win_size]))
...
class MSLSegLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        index = index * self.step        # 窗口有重叠
        if self.flag == "train":
            return (np.float32(self.train[index:index + self.win_size]), np.float32(self.train[index + self.win_size:index + 2*self.win_size]))
        elif (self.flag == 'val'):
            return (np.float32(self.val[index:index + self.win_size]), np.float32(self.val[index + self.win_size:index + 2*self.win_size]))
        elif (self.flag == 'test'):
            return (np.float32(self.test[index:index + self.win_size]),
                    np.float32(self.test[index + self.win_size : index + 2 * self.win_size]),
                    np.float32(self.test_labels[index + self.win_size : index + 2*self.win_size]))
...
class SWATSegLoader(Dataset):
    def __init__(self, args, root_path, win_size, step=1, flag="train"):
        ...
        self.win_size = win_size
        self.scaler = StandardScaler()
        data = np.load(os.path.join(root_path, "SWAT_train.npy"))
        self.scaler.fit(data)
        data = self.scaler.transform(data)
        test_data = np.load(os.path.join(root_path, "SWAT_test.npy"))
        self.test = self.scaler.transform(test_data)
        self.train = data
        data_len = len(self.train)
        self.val = self.train[(int)(data_len * 0.8):]
...
class DATASegLoader(Dataset):
    def __init__(self, args, root_path, data_path, win_size, step=1, flag="train"):
        ...
        # 将 cols 列中包含 'label' 的字符串都替换成 'label'
        ...
        if 'label' in data.columns:
            label_col = data.pop('label')  # 删除并保存
            data['label'] = label_col  # 添加到最后
        # 最终结果为纯数据宽表，无日期列
