...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model, path, file_name=None):
        score = -val_loss
        if self.best_score is None:
            self.best_score = score
            self.save_checkpoint(val_loss, model, path, file_name=file_name)
...
# 调整序列长度使其一样长
def adjustment_len(gt, pred):
    remaining_length = len(gt) - len(pred)
